...
const Product_List = () => {
  ...
  {products.map((prod) => (
    <tr key={prod._id} className="border-b border-white hover:bg-gray-700">
      ...
      <td className="py-2 align-top break-words whitespace-pre-line max-w-xs ">
        {prod.description}
        {prod.type === 'simple' && (
          <div className="mt-1 text-sm text-green-300">
            <strong>Selling Price:</strong> ₹{prod.simple.sellingPrice}
          </div>
        )}
      </td>
      <td className="py-2">
        <div className="flex justify-center items-center gap-4">
          <Link to={`/api/admin/edit-product/${prod._id}`}>
            <MdEdit className="text-blue-400 hover:text-blue-600 text-xl cursor-pointer" />
          </Link>
